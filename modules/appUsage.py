...
def create(fromTimestamp, toTimestamp, database, group):
    # Prepare output file
    appfocusFile = open('output/data/appUsage.js', 'w', encoding="utf-8")
    appfocusFile.write('var appUsageData = [')
    ...
    appFocus = cur.execute("SELECT * \
                             FROM APOLLO \
                             WHERE Activity = 'Application Usage' \
                             AND Key >= '{0}' AND Key <= '{1}' \
    ...
    for event in appFocus:
        ...
        title = "<b>App Usage</b><br />{0}<br />{1} - {2}<br />".format(bundle, startString, endString)
        output += "{" + \
                  "start: '{0}', " \
                  "end: '{1}', " \
                  "type:'{2}', " \
